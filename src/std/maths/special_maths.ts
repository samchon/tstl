...
namespace std
{
	/* ======================================
		BESSELS
			...
			- sph_bessel
			- UTILITIES
		 * reference: https://github.com/SheetJS/bessel/blob/master/bessel.js
	========================================
		cyl_bessel_i
	--------------------------------------------------------- */
	export function cyl_bessel_i(v: number, x: number): number
	{
		...
		let bi: number = 1.0;
		let bim: number = 0.0;
		let m: number = 2 * Math.round((v + Math.round(Math.sqrt(40 * v))) / 2);
