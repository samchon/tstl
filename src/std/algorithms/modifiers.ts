...
namespace std
{
	...
	export function copy_backward
		<T, BidirectionalIterator extends IBidirectionalIterator<T>, OutputIterator extends base.ILinearIterator<T>>
		(first: BidirectionalIterator, last: BidirectionalIterator, result: OutputIterator): OutputIterator
	{
		...
		for (; !last.equals(first); last = last.prev() as BidirectionalIterator)
		{
			result.value = last.value;
			result = result.prev() as OutputIterator;
		}
		return result;
	}
