...
namespace std
{
		...
		implements base.ITreeMap<Key, T>
	{
		 ...
		 */
		public constructor(begin: IForwardIterator<Pair<Key, T>>, end: IForwardIterator<Pair<Key, T>>);
		...
		public constructor
		(
			begin: IForwardIterator<Pair<Key, T>>, end: IForwardIterator<Pair<Key, T>>, compare: (x: Key, y: Key) => boolean
		);
		...
		public constructor(...args: any[])
		{
			...
			{
				// RANGE CONSTRUCTOR
				let first: IForwardIterator<Pair<Key, T>> = args[0]; // PARAMETER 1
				let last: IForwardIterator<Pair<Key, T>> = args[1]; // PARAMETER 2
		 ...
		 * @hidden
		 */
		protected _Insert_by_range<L extends Key, U extends T, InputIterator extends IForwardIterator<Pair<L, U>>>
