...
namespace std.base
{
		...
		extends Container<Pair<Key, T>>
	{
		 ...
		 * @inheritdoc
		 */
		public assign<L extends Key, U extends T, InputIterator extends IForwardIterator<Pair<L, U>>>
		 ...
		 */
		public insert<L extends Key, U extends T, InputIterator extends IForwardIterator<Pair<L, U>>>
			(first: InputIterator, last: InputIterator): void;
		...
		public insert(...args: any[]): any
		{
			...
			else if (args.length == 2 && args[0].next instanceof Function)
			{
					...
					(first instanceof MapIterator || (first as MapIterator<Key, T>).source() != this))
				{
					...
					return this._Insert_by_range(first, args[1]);
				}
				else if (first instanceof MapIterator && first.source() == this)
				{
					...
					if (first instanceof MapReverseIterator)
					{
						...
						first = (first as MapReverseIterator<Key, T>).base().prev();
					}

					// INSERT AN ELEMENT
					if (args[1] instanceof Pair)
						ret = this._Insert_by_hint(first as MapIterator<Key, T>, args[1]);
					else
						ret = this._Insert_by_hint_with_tuple(first as MapIterator<Key, T>, args[1]);
		 ...
		 * @hidden
		 */
		protected abstract _Insert_by_range<L extends Key, U extends T, InputIterator extends IForwardIterator<Pair<L, U>>>
			(first: InputIterator, last: InputIterator): void;
		...
		{
			if (args.length == 1 && (args[0].next instanceof Function == false || (args[0] as MapIterator<Key, T>).source() != this))
