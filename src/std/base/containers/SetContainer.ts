		...
		public insert(...args: any[]): any
		{
			...
			else if (args.length == 2 && args[0].next instanceof Function)
			{
				...
				if (args[1].next instanceof Function && 
					(first instanceof SetIterator == false || (first as SetIterator<T>).source() != this))
				{
					// IT DOESN'T CONTAIN POSITION
					// RANGES TO INSERT ONLY
					return this._Insert_by_range(first, args[1]);
				}
				else if (first instanceof SetIterator && first.source() == this)
				{
					...
					if (first instanceof SetReverseIterator)
					{
						...
						first = first.base().prev();
					}

					// INSERT AN ELEMENT
					ret = this._Insert_by_hint(first as SetIterator<T>, args[1]);
