        ...
        >
{
        ...
        if (pos.equals(this.end()) === true) {
            ...
            if (size > this.capacity_) {
                ...
                deque._Reserve(
                    Math.max(
                        size,
                        Math.floor(this.capacity_ * Deque.MAGNIFIER),
                    ),
                );
    ...
    >(pos: Deque.Iterator<T>, first: InputIterator, last: InputIterator): void {
        ...
        for (; !first.equals(last); first = first.next()) {
            if (
                ...
                const spliced_array: T[][] = this.matrix_.splice(
                    ++indexes.first,
                );
    ...
    protected _Erase_by_range(
        ...
        while (size !== 0) {
            ...
            const indexes: Pair<number, number> = this._Fetch_index(
                first.index(),
            );
