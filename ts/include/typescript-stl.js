...
(function (std) {
    ...
    (function (example) {
        function test_deque() {
            var it = std.remove_if(std.deque.begin(), std.deque.end(), function () { return true; });
            console.log(it.index);
            std.remove(std.deque.begin(), std.deque.end(), 1);
            std.remove(std.deque.begin(), std.deque.end(), 2);
            for (var i = 0; i < 10; i++)
                std.deque.push_back(i);
            var it = std.deque.begin().advance(3);
            it = std.deque.erase(it); // erase 3
            console.log(it.value); // print 4
            it = std.deque.begin().advance(2);
            it = std.deque.insert(it, -1); // insert -1
            console.log(it.next().value); // print 2
            it = std.deque.begin().advance(6);
            it = std.deque.erase(it, it.advance(3)); // erase from 6 to 9
            //console.log(it.value); // print 9
            console.log(it.equal_to(std.deque.end()));
            console.log("-------------------------------------");
            for (var it_1 = std.deque.begin(); !it_1.equal_to(std.deque.end()); it_1 = it_1.next())
